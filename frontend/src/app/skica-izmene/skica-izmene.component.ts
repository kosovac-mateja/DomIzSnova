...
@Component({
  selector: 'app-skica-izmene',
  templateUrl: './skica-izmene.component.html',
  styleUrls: ['./skica-izmene.component.css'],
})
export class SkicaIzmeneComponent implements OnInit {
  constructor(
  ...
  ngOnInit(): void {
    const idSkica = sessionStorage.getItem('idSkica');
    this.skicaServis.dohvatiSkicu(idSkica).subscribe((skica: Skica) => {
      this.skica = skica;
      this.brProstorija = skica.koordinate.length;
      this.sirine = [];
      this.visine = [];
      for (let i = 0; i < this.brProstorija; i++) {
        ...
        this.sirine.push(skica.dimenzije[i].sirina / 15);
        this.visine.push(skica.dimenzije[i].visina / 15);
      }
  ...
  izmeni() {
    ...
    this.skicaServis
      .izmeniSkicu(this.skica._id, koord, dim, koordinateVrata)
      .subscribe((res) => {
        alert('Skica je uspesno izmenjena');
